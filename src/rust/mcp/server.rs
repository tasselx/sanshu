        ...
        if self.is_tool_enabled("ji") {
            let ji_schema = serde_json::json!({
                ...
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["记忆", "回忆", "整理", "预览整理", "应用整理", "备份列表", "恢复备份", "导出备份", "列表", "预览相似", "配置", "删除"],
                        "description": "操作类型（必须从枚举中逐字选择，禁止自造英文动词）：记忆(添加) | 回忆(查询) | 整理(去重) | 预览整理(候选预览) | 应用整理(按计划清理) | 备份列表 | 恢复备份 | 导出备份 | 列表(全部记忆) | 预览相似(检测相似度) | 配置(获取/更新) | 删除(移除记忆)"
                    },
                    "project_path": {
                        "type": "string",
                        "description": "项目路径（必需）"
