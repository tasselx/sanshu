...
use super::api;
use super::types::{
    ClearCacheRequest, ClearCacheResult, IconCacheStats, IconConfig, IconContentRequest,
    IconContentResult, IconFormat, IconItem, IconSaveItem, IconSaveRequest, IconSaveResult,
    IconSearchRequest, IconSearchResult,
};
use crate::log_debug;
...
pub async fn search_icons(request: IconSearchRequest) -> Result<IconSearchResult, String> {
    ...
    api::search_icons(request).await.map_err(|e| {
...
async fn save_single_icon(
    ...
    if *format == IconFormat::Svg || *format == IconFormat::Both {
        ...
        fs::write(&svg_path, &cleaned_svg).map_err(|e| format!("写入 SVG 文件失败: {}", e))?;
...
fn convert_svg_to_png(svg_content: &str, output_path: &PathBuf, size: u32) -> Result<(), String> {
    ...
    // 创建目标尺寸的画布
    let pixmap_size = tiny_skia::IntSize::from_wh(size, size).ok_or("无效的尺寸")?;
    let mut pixmap =
        tiny_skia::Pixmap::new(pixmap_size.width(), pixmap_size.height()).ok_or("创建画布失败")?;
...
/// 获取图标工坊配置（从主配置系统读取）
#[tauri::command]
pub fn get_icon_config(state: tauri::State<crate::config::AppState>) -> Result<IconConfig, String> {
    let config = state
        .config
        .lock()
    ...
    Ok(IconConfig {
        default_save_path: mcp
            .icon_default_save_path
            .clone()
