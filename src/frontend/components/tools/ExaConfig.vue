...
async function loadConfig() {
  try {
    const res = await invoke('get_exa_config') as { api_key?: string }
...
async function saveConfig() {
  ...
  try {
    await invoke('save_exa_config', { apiKey: config.value.api_key })
    message.success('Exa 配置已保存')
  }
  catch (err) {
...
async function runTest() {
  ...
  try {
    const res = await invoke('test_exa_connection', {
      apiKey: key,
    }) as any
    ...
    <n-scrollbar class="config-scrollbar">
      <n-space vertical size="large" class="config-content">
        ...
        <n-alert type="info" :bordered="false" class="intro-alert">
          ...
          Exa 提供基于神经网络嵌入的语义搜索引擎 API，与官方 exa-mcp-server 同源实现。新注册账号赠送 $10 免费额度。
        </n-alert>

        <!-- 认证设置 -->
        <ConfigSection title="认证设置" description="配置 Exa API Key（必填）">
          ...
          <n-space size="small" vertical>
            <div class="text-xs text-on-surface-secondary">
              <n-tag size="small" type="success" :bordered="false">
                Search
              </n-tag> 约 $0.005/次（keyword 约 $0.0025/次）
              <n-tag size="small" type="info" :bordered="false" class="ml-2">
                Contents
              </n-tag> 约 $0.001/页
            </div>
            <div class="text-xs opacity-50">
              新注册账号赠送 $10 免费额度，详见 exa.ai/pricing
            </div>
          </n-space>
        </div>
      </n-space>
