...
// 面板显示模式
const displayMode = computed<'normal' | 'guide-sou' | 'guide-ace' | 'fast-context'>(() => {
  if (!props.souEnabled)
    return 'guide-sou'
  if (!props.acemcpConfigured)
    return 'guide-ace'
  // fast-context 不依赖本地索引，显示简洁的就绪状态
  if (props.backend === 'fast_context')
    return 'fast-context'
  return 'normal'
})
...
<template>
  ...
  <div v-if="shouldShow" class="zhi-index-panel">
    ...
    >
      ...
      <div class="guide-content">
        <span class="guide-text">配置搜索后端以启用代码索引</span>
        <n-button text type="primary" size="tiny" @click="handleOpenSettings">
          前往配置
          <template #icon>
            <div class="i-carbon-arrow-right" />
    ...
    <!-- ==================== fast-context 就绪模式 ==================== -->
    <div
      v-else-if="displayMode === 'fast-context'"
      class="panel-guide panel-guide--ready"
    >
      <div class="guide-icon guide-icon--ready">
        <div class="i-carbon-flash text-lg text-emerald-400/80" />
      </div>
      <div class="guide-content">
        <span class="guide-text">fast-context 已就绪</span>
        <n-button text type="primary" size="tiny" @click="handleOpenSettings">
          设置
          <template #icon>
